...
class Library:
    ...
    def __str__(self):
        ...
        for i in range(len(self.book_list)):
            string+= f"\nTitle : {self.book_list[i].title}\nAuthor : {self.book_list[i].author}\nPublication year : {self.book_list[i].publication}\nId : {self.book_list[i].id}\n"
            string+= f"{seperator()}"
        return string
    ...
    def search_for_book(self,query):
        for i in range(len(self.book_list)):
            if self.book_list[i].id==query or self.book_list[i].title==query or self.book_list[i].author==query or self.book_list[i].publication==query:
                return self.book_list[i]
        return 404
